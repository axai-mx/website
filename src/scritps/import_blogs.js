...
rows.forEach(row => {
  ...
  const folder = row.status ? '/../blog/' : '/../../drafts/';
  ...
  const image = db
    .prepare(
      `SELECT filename, uri FROM field_data_field_imagen_principal i
      INNER JOIN file_managed f ON f.fid = i.field_imagen_principal_fid
      WHERE i.entity_id = ?`
    )
    .get(row.nid);
  if (image) {
